...
def test_home(page: Page, live_server_url: str):
    ...
    expect(page).to_have_title("Quantum Intelligence Group - Contract Analyst")
...
def test_chat(sized_page: Page, live_server_url: str):
    ...
    page.get_by_placeholder("Type a new question (e.g. Summarize the contract)").click()
    page.get_by_placeholder("Type a new question (e.g. Summarize the contract)").fill(
...
def test_chat_customization(page: Page, live_server_url: str):
    ...
    expect(page.get_by_text("Whats the City of Dallas contract?")).to_be_visible()
    expect(page.get_by_text("The capital of France is Paris.")).to_be_visible()
    expect(page.get_by_role("button", name="Clear chat")).to_be_enabled()
...
def test_ask(sized_page: Page, live_server_url: str):
    ...
    page.get_by_placeholder("Example: Summarize the contract").click()
    page.get_by_placeholder("Example: Summarize the contract").fill("Whats the dental plan?")
    page.get_by_placeholder("Example: Summarize the contract").click()
    page.get_by_label("Submit question").click()
